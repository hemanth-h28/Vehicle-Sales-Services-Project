...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Ln(10);
        $this->Cell(0, 10, 'Product Details', 0, 1, 'C');
    }
...
if ($result->num_rows > 0) {
    $header = array('Product ID', 'Product Name', 'Product Description', 'Product Price');
    ...
    while ($row = $result->fetch_assoc()) {
        $pdf->Cell(40, 10, $row["proid"], 1);
        $pdf->Cell(40, 10, $row["proname"], 1);
        $pdf->Cell(40, 10, $row["prodesc"], 1);
